...
int _partition_move(vector<int> &arr, int start_index, int end_index)
{
    ...
    while (right >= left)
    {
        ...
        while (right >= left)
        {
            if (arr[right] < pivot)
            {
                // 最右边的数小于基准数 需要放入最左边的数的位置 left位置的数,初始基准数已经保存,所以直接覆盖
                arr[left] = arr[right];
    ...
    // 循环结束 将之前保存的基准数放到index位置 此时 左边数 <= arr[index] <= 右边数
    arr[index] = pivot;
...
初始假定待排数组的正数第一位 i 为最小值，倒数第一位 n-i-1 为最大值，然后遍历整个数组(包括i和 n-i-1)，找到待排数组中的最大最小值，更新指针 min_index和 max_index。
...
如果待排序列的倒数第一位 nums[n-i-1]是最小值呢？如果先交换的是最小值，在交换 nums[i] 和 nums[n-i-1] 后，不影响最大值nums[max_index] 和 nums[n-i-1] 的交换。如果先交换的是最大值，那交换后 n-i-1 处就不再是最小值，就会影响最小值的交换。所以先进行交换的要处理这个特殊情况。
...
void select_sort_plus(vector<int> &arr)
{
    ...
    for (int i = 0; i <= len / 2; i++)
    {
        ...
        {
            swap(arr[i], arr[min_index]);
            // 说明此次交换改变了最大数的下标 先进行交换的需要考虑特殊情况, 所以需要将最大数下标重新归位
            if (i == max_index)
            {
                max_index = min_index;
            }
...
void insert_sort_swap(vector<int> &arr)
{
    ...
    for (int i = 1; i < len; i++)
    {
        for (int j = i - 1; j >= 0; j--)
        {
            ...
            {
                // 因为前面是有序数组 比较是从后往前比较 只要最后比较的元素就绪 就结束当前循环
                break;
            }
        }
    }
...
// 快速排序 挖坑法 快速排序在每一轮挑选一个基准元素，并让其他比它大的元素移动到数列一边，比它小的元素移动到数列的
